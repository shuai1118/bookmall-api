...
import com.book.mall.api.mall.param.SaveMallUserAddressParam;
import com.book.mall.api.mall.param.UpdateMallUserAddressParam;
...
@RestController
@Api(value = "v1", tags = "6.图书商城个人地址相关接口")
@RequestMapping("/api/v1")
public class BookMallUserAddressAPI {

    @Resource
    private BookMallUserAddressService mallUserAddressService;

    @GetMapping("/address")
    @ApiOperation(value = "我的收货地址列表", notes = "")
    public Result<List<BookMallUserAddressVO>> addressList(@TokenToMallUser MallUser loginMallUser) {
    ...
    public Result<BookMallUserAddressVO> getMallUserAddress(@PathVariable("addressId") Long addressId,
                                                              @TokenToMallUser MallUser loginMallUser) {
        MallUserAddress mallUserAddressById = mallUserAddressService.getMallUserAddressById(addressId);
        BookMallUserAddressVO bookMallUserAddressVO = new BookMallUserAddressVO();
        BeanUtil.copyProperties(mallUserAddressById, bookMallUserAddressVO);
        if (!loginMallUser.getUserId().equals(mallUserAddressById.getUserId())) {
            return ResultGenerator.genFailResult(ServiceResultEnum.REQUEST_FORBIDEN_ERROR.getResult());
        }
        return ResultGenerator.genSuccessResult(bookMallUserAddressVO);
    }
