...
import com.book.mall.util.BookMallUtils;
...
public class BookAdminUploadAPI {

    private static final Logger logger = LoggerFactory.getLogger(BookAdminUploadAPI.class);
    ...
    public Result upload(HttpServletRequest httpServletRequest, @RequestParam("file") MultipartFile file, @TokenToAdminUser AdminUserToken adminUser) throws URISyntaxException {
        ...
        try {
            ...
            resultSuccess.setData(BookMallUtils.getHost(new URI(httpServletRequest.getRequestURL() + "")) + "/upload/" + newFileName);
    ...
    public Result uploadV2(HttpServletRequest httpServletRequest, @TokenToAdminUser AdminUserToken adminUser) throws URISyntaxException {
        ...
        for (int i = 0; i < multipartFiles.size(); i++) {
            ...
            try {
                ...
                fileNames.add(BookMallUtils.getHost(new URI(httpServletRequest.getRequestURL() + "")) + "/upload/" + newFileName);
            } catch (IOException e) {
                e.printStackTrace();
                return ResultGenerator.genFailResult("文件上传失败");
            }
