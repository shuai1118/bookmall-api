...
import com.book.mall.api.admin.param.IndexConfigAddParam;
import com.book.mall.api.admin.param.IndexConfigEditParam;
...
public class BookAdminIndexConfigAPI {

    private static final Logger logger = LoggerFactory.getLogger(BookAdminIndexConfigAPI.class);
    ...
    public Result list(@RequestParam(required = false) @ApiParam(value = "页码") Integer pageNumber,
        ...
        return ResultGenerator.genSuccessResult(bookMallIndexConfigService.getConfigsPage(pageUtil));
    }
    ...
    public Result save(@RequestBody @Valid IndexConfigAddParam indexConfigAddParam, @TokenToAdminUser AdminUserToken adminUser) {
        ...
        String result = bookMallIndexConfigService.saveIndexConfig(indexConfig);
    ...
    public Result update(@RequestBody @Valid IndexConfigEditParam indexConfigEditParam, @TokenToAdminUser AdminUserToken adminUser) {
        ...
        String result = bookMallIndexConfigService.updateIndexConfig(indexConfig);
        if (ServiceResultEnum.SUCCESS.getResult().equals(result)) {
            return ResultGenerator.genSuccessResult();
        } else {
            return ResultGenerator.genFailResult(result);
    ...
    public Result info(@PathVariable("id") Long id, @TokenToAdminUser AdminUserToken adminUser) {
        logger.info("adminUser:{}", adminUser.toString());
        IndexConfig config = bookMallIndexConfigService.getIndexConfigById(id);
        if (config == null) {
    ...
    public Result delete(@RequestBody BatchIdParam batchIdParam, @TokenToAdminUser AdminUserToken adminUser) {
        ...
        if (bookMallIndexConfigService.deleteBatch(batchIdParam.getIds())) {
            return ResultGenerator.genSuccessResult();
        } else {
